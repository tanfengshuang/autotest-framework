...
class rhts_linpack(test.test):
    ...
    def run_once(self, config='./linpack.cfg'):
        ...
        rhts_pkg_list = ['beakerlib', 'rhts_test_env']
        for pkg in rhts_pkg_list:
            pkg_url = cfg.get('rhts_test_env', '%s_url' % pkg)
            pkg_md5 = cfg.get('rhts_test_env', '%s_md5' % pkg)
            pkg = utils.unmap_url_cache(cachedir, pkg_url, pkg_md5)
            package.install(pkg)
