...
class Des(object):
    ...
    def create_Kn(self):
        ...
        """
        key = self.get_sub_list(self.PC1, virt_utils.String_to_BitList(self.getKey()))
        self.L = key[:28]
        self.R = key[28:]
        for i in range(16):
    ...
    def crypt(self, data, crypt_type=0):
        ...
        while i < len(data):
            ...
            block = virt_utils.String_to_BitList(data[i:i+8])
            pro_block = self.des_crypt(block,crypt_type)

            # Add the resulting block to our list
            result.append(virt_utils.BitList_to_String(pro_block))
            i += 8
