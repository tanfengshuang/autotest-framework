...
def run_tc_SAMID129292_edit_system(test, params, env):
	...
	try:
		...
		location_new="location_new"
                cmd="headpin -u admin -p admin system update --name=%s --org=%s --new_name=%s --description='%s' --location=%s"%(ee.systemname1, ee.default_org, ee.systemname2, description_new, location_new)
		(ret,output)=eu().runcmd(session,cmd,"register system")
