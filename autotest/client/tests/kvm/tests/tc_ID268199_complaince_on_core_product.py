...
def run_tc_ID268199_complaince_on_core_product(test, params, env):
	...
	try:   
		...
		factsupdate(session, "cpu.core(s)_per_socket","16")
		
		#list available entitlement pools
		#productid=ee.productid2cc
		productid = ee().get_newproduct("core")["productidstack"]
		productname = ee().get_newproduct("core")["productname"]
		...
		factsupdate(session, "cpu.core(s)_per_socket","24")
		checkstatus(session, "Insufficient")
		checkinstalled(session, "Partially Subscribed")		
