...
def run_tc_ID115143_disabled_healing_for_compliance_broken(test, params, env):
	...
	try:
		...
		check_compliance(session, productname)
		...
		if ret == 0 and "1" in output:
			logging.info("It's successful to check heal for broken compliance failed when autoheal is disabled.")
		else:
			raise error.TestFail("Test Failed - Failed to check heal for broken compliance failed when autoheal is disabled.")
	
	except Exception, e:
		logging.error(str(e))
		raise error.TestFail("Test Failed - error happened when check heal for broken compliance failed when autoheal is disabled:"+str(e))
	finally:
...
def check_compliance(session, productname):
