...
function verify_repomanifest(){
    ...
    while read repo
    do
        #d_echo "Filter the $relversion repo from manifest."
        #echo $repo| grep "\-$relversion\-" 
        #if [ $? -eq 0 ];then
...
function verify_content(){
    ...
    then
	cat $pkginvalid2
	echo "Above $pkginvl2len pkgs could not find in repo metadata!"
        echo "**************************************************"
        cat $pkginvalid3 
        echo "Above $pkginvl3len pkgs could not find in Packages Content Directory!"
        echo "**************************************************"
...
function verify_treeinfo()
{
     ...
     #d_echo "_pkgurl= $_pkgurl"
     #d_echo "_os_url= $_os_url"
     #d_echo "_cur_arch= $_cur_arch"
     ...
     then
          ...
          if eval $cmd | grep "kernel = images">/dev/null;
          then
...
{ 
	...
	case $content_level in
			...
			then
        		...
        		if [ ${checktreeinfo} -eq 0 ];then
        		    # verify treeinfo        
        		    verify_treeinfo $PkgUrl $CurrentArch
                            checktreeinfo=1
        		fi
    ...
    #content verify for each effective repo end
     ...
     #get original repo manifest, when several products bundled in one subscription, we need test all repos.
     ...
     echo  $__CurrentRelVer|grep 'Loaded plugins: product-id'
     if [ $? -eq 0 ];then
     ...
     #get repo manifest related the release version
     relversion=$(echo $CurrentRelVer | grep -Eo '^[0-9]')

     #get current base architecture
     __CurrentArch=$(python -c "import yum; yb = yum.YumBase(); print(yb.conf.yumvar)['basearch']")
     echo  $__CurrentArch |grep 'Loaded plugins: product-id'
     if [ $? -eq 0 ];then
     ...
     #get repo manifest related CurrentRelVer on CurrentArch
     repomanifest=$(mktemp)
     if [[ $CurrentArch == *ppc* ]]
     then
	...
	grep "rhel\-$relversion" $orgrepomanifest| grep -v "power" | sed -e "s/^ *//" -e "s/ *$//" > $repomanifest
     ...
     d_echo "**************************************************"
     content_test $repomanifest $CurrentRelVer $CurrentArch $content_level $relserver 
     d_echo "**************************************************"
