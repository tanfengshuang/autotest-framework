...
def run_tc_ID115170_register_with_org_option_via_GUI(test, params, env):
	...
	try:
		...
		egu().select_org_and_register()
		if eu().sub_checkidcert(session):
			logging.info("It's successful to register_with_org_option_via_GUI.")
		else:
			raise error.TestFail("Test Faild - Failed to register_with_org_option_via_GUI")
		
	except Exception, e:
		logging.error(str(e))
		raise error.TestFail("Test Failed - error happened to register_with_org_option_via_GUI:" + str(e))
	finally:
		egu().capture_image("register_with_org_option_via_GUI")
