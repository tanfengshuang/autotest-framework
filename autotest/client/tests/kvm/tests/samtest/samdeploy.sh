...
sed -i 's/^ *baseurl*=.*/baseurl= https:\/\/cdn.rcm-qa.redhat.com/' /etc/rhsm/rhsm.conf
...
echo $cmd
eval $cmd
pool_id=$(grep --max-count=1 -A 1 RH0103708 $tmp | awk -F":" '/Pool/{print $2;}' | sed 's/^[ \t]*//;s/[ \t]*$//')
if [ $pool_id'X' = 'X' ]
then
...
#line=$(($(grep -n "^\[rhel-6-server-beta-rpms\]" /etc/yum.repos.d/redhat.repo | awk -F":" '{print $1}')+3))
#sed -i "$line s/0/1/" /etc/yum.repos.d/redhat.repo
#line=$(($(grep -n "^\[rhel-6-server-optional-beta-rpms\]" /etc/yum.repos.d/redhat.repo | awk -F":" '{print $1}')+3))
#sed -i "$line s/0/1/" /etc/yum.repos.d/redhat.repo
##################################
...
##########################
#modified by wanghui
#if [ -z "$(echo $repolist|grep rhel-6-server-beta-rpms)" ] || [ -z "$(echo $repolist|grep rhel-6-server-rpms)" ] || [ -z "$(echo $repolist|grep sam)" ] || [ $(repoquery -a --repoid=rhel-6-server-beta-rpms|wc -l) == 0 ] || [ $(repoquery -a --repoid=rhel-6-server-rpms|wc -l) == 0 ] || [ $(repoquery -a --repoid=sam|wc -l) == 0 ]
if  [ -z "$(echo $repolist|grep rhel-6-server-rpms)" ] || [ -z "$(echo $repolist|grep sam)" ] || [ $(repoquery -a --repoid=rhel-6-server-rpms|wc -l) == 0 ] || [ $(repoquery -a --repoid=sam|wc -l) == 0 ]
##########################
then
...
cmd="katello-configure --deployment=sam"
echo $cmd
eval $cmd
