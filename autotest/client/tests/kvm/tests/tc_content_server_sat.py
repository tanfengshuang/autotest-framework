...
def run_tc_content_server_sat(test, params, env):
    ...
    try:
        try:
            ...
            relserver = params.get("relserver")
            baseurl = params.get("baseurl")
            hostname = params.get("hostname")

            if ("stage" in hostname):
                username = 'stage_test_101'
                password = 'redhat'
                sku = "MCT0370"
                ########################
                # if the platform is s390x sku needs to be "MCT0370,RH0416249"
                ########################
            else:
