...
def run_tc_ID178118_GUI_display_orgname_and_identity(test, params, env):
	...
	try:
		...
		egu().click_facts_view_tree("system")
		...
		if egu().check_system_uuid_displayed_in_facts(session):
			logging.info("It's successful to check system_uuid displayed in system facts in GUI ")
		else:
			raise error.TestFail("Test Faild - Failed to check system_uuid displayed in system facts in GUI")
	except Exception, e:
		logging.error(str(e))
		raise error.TestFail("Test Failed - error happened to display_orgname_and_identity in GUI:" + str(e))
	finally:
		egu().capture_image("display_orgname_and_identity")
