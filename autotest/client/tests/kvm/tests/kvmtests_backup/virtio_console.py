    ...
    def _restore_vm():
        ...
        try:
            session = vm.wait_for_login(
                                timeout=float(params.get("boot_timeout", 100)))
        ...
        sserial = vm.wait_for_serial_login(
                                timeout=float(params.get("boot_timeout", 20)))
        ...
        if (match is None) or (match != 0):
            ...
            try:
                ...
                if (match is None) or (match == 0):
                    vm[1].close()
                    vm[1] = vm[0].wait_for_login(
                                timeout=float(params.get("boot_timeout", 5)))
