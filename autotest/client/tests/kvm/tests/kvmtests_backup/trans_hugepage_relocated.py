...
def run_trans_hugepage_relocated(test, params, env):
    ...
    def nr_hugepage_check(sleep_time, wait_time):
        time_last = 0
        while True:
            value = int(virt_utils.get_mem_status("AnonHugePages"))
    ...
    free_memory = virt_utils.get_mem_status("MemFree")
    total_memory = virt_utils.get_mem_status("MemTotal")
    swap = virt_utils.get_mem_status("SwapTotal")
    hugepage_size = virt_utils.get_mem_status("Hugepagesize")
    mem = params.get("mem")
    vmsm =  int(mem) + 128
    ...
    bg = virt_test_utils.BackgroundTest(nr_hugepage_check, (s_time, w_time))
    bg.start()
