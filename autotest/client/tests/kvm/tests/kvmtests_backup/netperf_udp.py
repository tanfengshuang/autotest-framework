...
def run_netperf_udp(test, params, env):
    ...
    def get_remote_host_session():
        dsthostssh = virt_utils.remote_login("ssh", dsthost, 22, "root",
                                            PASSWD, "#", timeout=30)
        if dsthostssh:
    ...
    def scp_to_remote(local_path="", remote_path=""):
        virt_utils.scp_to_remote(dsthost, 22, "root",PASSWD, local_path,
                                remote_path)
    ...
    dsthostssh = virt_utils.wait_for(get_remote_host_session, 120, 0, 2)
    if not dsthostssh:
