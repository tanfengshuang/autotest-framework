...
from autotest_lib.client.virt import virt_test_utils, virt_utils, virt_env_process
...
def run_stress_kernel_compile(tests, params, env):
    ...
    def kernelcompile(se, vmnum):
        ...
        try:
            ...
            s, o = virt_test_utils.ping(ip, count=10, timeout=30)
            if s != 0:
    ...
    virt_env_process.preprocess_vm(tests, params, env, vm_name)
    ...
    while num <= (num_vm):
        ...
        virt_env_process.preprocess_vm(tests, vm_params, env, vm_name)
        params['vms'] += " " + vm_name
    ...
    try:
        ...
        for num, session in enumerate(sessions):
            bg.append(virt_test_utils.BackgroundTest(kernelcompile, (session,num+1)))
            bg[num].start()
