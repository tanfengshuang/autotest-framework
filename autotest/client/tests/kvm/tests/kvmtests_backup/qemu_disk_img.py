...
from autotest_lib.client.virt import virt_vm, virt_utils, virt_test_utils
...
def run_qemu_disk_img(test, params, env):
    ...
    def _check_file(img_name, file_check, file_create=None):
        ...
        try:
            ...
            if not virt_utils.wait_for(vm.is_dead, login_timeout):
    ...
    def convert_test():
        ...
        img_filename = virt_vm.get_image_filename(params, test.bindir)
        ...
        if snapshot_name:
            ...
            snapshot_filename = virt_vm.get_image_filename(params, test.bindir)
            virt_test_utils.create_image(cmd, snapshot_filename,
        ...
        convert_filename = virt_vm.get_image_filename(params, test.bindir)
        virt_test_utils.convert_image(cmd, img_filename, image_format,
    ...
    def commit_test():
        ...
        base_filename = virt_vm.get_image_filename(params, test.bindir)
        ...
        snapshot_filename = virt_vm.get_image_filename(params, test.bindir)
        virt_test_utils.create_image(cmd, snapshot_filename, snapshot_format,
                                    base_img=base_filename)
        _check_file(snapshot_filename, file_orig, file_create=file_sn)

        # Commit image
        virt_test_utils.commit_image(cmd, snapshot_filename, snapshot_format)
    ...
    def _create_snapshot_image_file(name, file_sn, base_img):
        ...
        params['image_format'] = snapshot_format
        sn_filename = virt_vm.get_image_filename(params, test.bindir)
        virt_test_utils.create_image(cmd, sn_filename, snapshot_format,
                                     base_img=base_img)
    ...
    def _do_rebase(from_file, to_file, file_size=None):
        ...
        virt_test_utils.rebase_image(cmd, from_file, to_file,
    ...
    def rebase_test():
        ...
        if name_sn3 and new_disk:
            ...
            new_filename = virt_vm.get_image_filename(params, test.bindir)
            virt_test_utils.create_image(cmd, new_filename, image_format,
                                        img_size = image_size)
            ...
            virt_test_utils.rebase_image(cmd, sn3_filename, sn2_filename,
                             snapshot_format, snapshot_fmt=snapshot_format)
