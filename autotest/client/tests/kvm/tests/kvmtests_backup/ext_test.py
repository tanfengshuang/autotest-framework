...
def run_ext_test(test, params, env):
    ...
    get_tm_list_cmd = virt_test_utils.fix_atest_cmd(atest_basedir,
                                                params.get('get_tm_list_cmd'),
                                                asvr_ip)

    asvr_session = virt_test_utils.get_svr_session(asvr_ip,
                                                  usrname=asvr_user,
                                                  passwd=asvr_passwd,
                                                  prompt=prompt)
    ...
    try:
        ...
        tm_list = virt_test_utils.get_svr_tm_lst(asvr_session, get_tm_list_cmd)
        ...
        stat_tm_cmd = virt_test_utils.fix_atest_cmd(atest_basedir,
                                                   stat_tm_cmd,
        ...
        if not virt_utils.wait_for(is_tm_ready, asvr_job_timeout, 5.0, 5.0):
        ...
        query_job_cmd = virt_test_utils.fix_atest_cmd(atest_basedir,
        ...
        if not virt_utils.wait_for(is_job_in_list, asvr_job_timeout, 60.0, 3.0):
            raise error.TestFail("Failed to list the job, id: %s"% str(job_id))

        logging.debug("Waiting for job to finish...")
        if not virt_utils.wait_for(lambda: not is_job_in_list(),
                                    asvr_job_timeout,
                                    60.0, 10.0):
        ...
        stat_job_cmd = virt_test_utils.fix_atest_cmd(atest_basedir,
...
##
        ...
        if params.get('wait_tm_cleanup') == 'yes':
            ...
            if not virt_utils.wait_for(is_tm_ready,asvr_job_timeout,10.0, 10.0):
                raise error.TestError("The test machine is not ready.")
