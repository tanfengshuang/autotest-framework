...
def run_rh_kernel_update(test, params, env):
    ...
    def install_rpm_from_brew(session, url, upgrade=False):
        ...
        if upgrade:
            ...
            cmd = "rpm -ivhf %s" % url
        s, o = session.get_command_status_output(cmd, timeout=600)
    ...
    def get_kernel_rpm_link():
        ...
        if method == "link":
            ...
            buildinfo = utils.system_output("brew buildinfo %s" % build, timeout=360)
            ...
            kernel_version = re.findall(debug_re, kernel_url)[0]
    ...
    def get_guest_kernel_version():
        s, o = session.get_command_status_output("uname -r")
        return o.strip()

    def virtio_driver_installed():
        s, o = session.get_command_status_output("lsmod | grep virtio")
    ...
    try:
        ...
        s, o = session.get_command_status_output(cp_grubcf_cmd)
        if s != 0:
            logging.error(o)
        ...
        if check_kernel_version(kernel_version, guest_version):
            ...
            if firmware_rpm is not None:
                ...
                install_rpm_from_brew(session, firmware_rpm, upgrade=False)
            ...
            if s != 0:
                logging.error(o)
                raise error.TestError("Fail to backup the grub.cfg after update kernel")
        ...
        if install_virtio == "yes":
            ...
            s, o = session.get_command_status_output(initrd_prob_cmd)
            if s != 0:
                logging.error(o)
            ...
            restore_initrd_cmd = "/bin/cp %s-bk %s" % (initrd_path, initrd_path)

            # backup initrd file
            s, o = session.get_command_status_output(cp_initrd_cmd, timeout=200)
            if s != 0:
                logging.error(o)
            # update initrd file
            s, o = session.get_command_status_output(mkinitrd_cmd, timeout=200)
            if s != 0:
                logging.error(o)
        ...
        if ifupdatekernel:
            ...
            s, o = session.get_command_status_output(make_def_cmd)
            if s != 0:
                logging.error(o)
        ...
        s, o = session.get_command_status_output(update_kernel_cmd)
        if s != 0:
            logging.error(o)
        ...
        if verify_virtio == "yes":
            ...
            if not virtio_driver_installed():
                raise error.TestFail("Fail to verify the installation of virtio"
                                     " drivers.")
    except Exception, e:
        if count == 4 or count == 3 or count == 1:
            # restore grub.cfg
            s, o = session.get_command_status_output(restore_grubcf_cmd, timeout=100)
            if s != 0:
                logging.error(o)
            raise error.TestFail(e)
        elif count == 2:
            # restore initrd file
            s, o = session.get_command_status_output(restore_initrd_cmd, timeout=200)
            if s != 0:
                logging.error(o)
            raise error.TestFail(e)
