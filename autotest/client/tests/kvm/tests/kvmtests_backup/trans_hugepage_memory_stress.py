...
def run_trans_hugepage_memory_stress(test, params, env):
    ...
    try:
        ...
        mem_free = int(virt_utils.get_mem_status('MemFree')) / 1024
        mem_swap = int(virt_utils.get_mem_status('SwapFree')) / 1024
        hugepage_size = int (virt_utils.get_mem_status('Hugepagesize')) / 1024
        ...
        nr_ah.append(int(virt_utils.get_mem_status('AnonHugePages')))
        if nr_ah[0] <= 0:
        ...
        virt_test_utils.run_sub_test(test, params, env,
