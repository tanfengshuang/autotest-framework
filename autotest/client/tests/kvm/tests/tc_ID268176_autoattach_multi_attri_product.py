...
def run_tc_ID268176_autoattach_multi_attri_product(test, params, env):
	...
	cmd="cp -rf \"/etc/pki/product/tmp/900.pem\" \"/etc/pki/product/\""
	...
	try:   
		...
		if ret == 0 and "Subscribed" in output:
			logging.info("It's successful to auto attach for multiattribute product .")
		else:
			raise error.TestFail("Test Failed - Failed to auto attach for multiattribute product.")
