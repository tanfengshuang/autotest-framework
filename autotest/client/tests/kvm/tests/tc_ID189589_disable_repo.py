...
def run_tc_ID189589_disable_repo(test, params, env):
	...
	try:
		...
		if ret == 0 and productrepo in output:
			...
			(ret,output)=eu().runcmd(session,cmd,"disable the repo %s"%productrepo)
			if ret == 0 and "Repo '%s' is disabled for this system."%productrepo in output:
				logging.info("It's successful to disable the repo %s."%productrepo)
			else: 
				raise error.TestFail("Test Failed - Failed to disable the repo %s."%productrepo)
		else:
			raise error.TestFail("Test Failed - Fail to check the repo %s which should be available." %productrepo)

	except Exception, e:
		logging.error(str(e))
		raise error.TestFail("Test Failed - error happened to disable a repo:"+str(e))
	finally:
