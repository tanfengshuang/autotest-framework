...
class UnattendedInstallConfig(object):
    ...
    def __init__(self, test, params):
        ...
        self.host_ip = params.get("srchost")
    ...
    def render_answer_file(self):
        ...
        def replace_virtio_key(contents, dummy_re, attribute_name):
            ...
            if re.search(dummy_re, contents):
                if self.install_virtio == "yes":
                    ...
                    elif driver.endswith("netkvm.inf"):
                        driver += ' PCI\VEN_1AF4^&DEV_1000^&SUBSYS_00011AF4'
                    else:
                        try:
        ...
        # Add host ip to socket programs
        unattended_contents = re.sub("finish.exe", "finish.exe %s" % host_ip,
                                     unattended_contents)
        unattended_contents = re.sub("autotest_host_ip",  host_ip,
                                      unattended_contents)
    ...
    def setup_url(self):
        ...
        kernel_fetch_cmd = "wget -q %s/%s/%s -O %s" % (self.url, self.boot_path,
                                                 os.path.basename(self.kernel),os.path.basename(self.kernel))
        initrd_fetch_cmd = "wget -q %s/%s/%s -O %s" % (self.url, self.boot_path,
                                                 os.path.basename(self.initrd),os.path.basename(self.initrd))
...
def run_unattended_install(test, params, env):
    ...
    #replaced_append = "ks=cdrom:/dev/sr0 nicdelay=60 console=ttyS0,115200 console=tty0 root=live:%s/LiveOS/squashfs.img" % RHEL_URL
    ...
    install_timeout = float(params.get("install_timeout", 60)) * 60 
    post_install_delay = int(params.get("post_install_delay", 0))
    port = vm.get_port(int(params.get("guest_port_unattended_install", 12323)))
    ...
    # Set the guest client connection default value to None
    client = None
