...
def run_tc_content_legacy_desktop(test, params, env):
	...
	try:
		try:         
			...
			release = ee().get_env(params)["releaselist"]
	
	                #[B]Run the test
			(prodsku, basesku) = ep().cnt_get_prodsku_and_basesku(sku)
			ep().cnt_rhel_test(session, vm, username, password, prodsku, basesku, productid_cont, content_level, "", release)
