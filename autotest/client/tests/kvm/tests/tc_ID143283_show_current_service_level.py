...
def run_tc_ID143283_show_current_service_level(test, params, env):
	...
	try:
		...
		(ret,output)=eu().runcmd(session,cmd,"autosubscribe with one service-level")
			...
			(result,outputnew)=eu().runcmd(session,cmd,"display current service-level")
			...
			if result == 0 and "Current service level: %s" %service_level in outputnew:
				...
				raise error.TestFail("Test Failed - Failed to display current service level.")
                else:
                        raise error.TestFail("Test Failed - Failed to auto subscribe with one service level.")

	except Exception, e:
		logging.error(str(e))
		raise error.TestFail("Test Failed - error happened when do display current service level:"+str(e))
