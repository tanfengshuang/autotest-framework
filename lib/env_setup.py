...
sys.path.insert(0, os.path.join(client_dir, 'tests/kvm'))
import common, kvm_config, kvm_utils
sys.path.pop(0)
...
def directory_setup(dict):
    ...
    try:
        ...
        link_if_not_exist(image_dir_path,image_link_path)
        ...
        def mount_dir(src, dest):
            ...
            for line in result:
                if src in line and dest in line:
                    ...
                    break;
            if mount == True:
                logging.info("%s already mounted to %s" % (src, dest))
            else:
                commands.getoutput("mount %s %s -o ro" % (src, dest))
                logging.info("Success to mount %s to %s" % (src, dest))

        nfs_src = dict.get("nfs_src")
        mount_dir(nfs_src, iso_dir_path)
        mkdir_if_not_exist("/mnt/windows")
        mkdir_if_not_exist("/mnt/images")
        mount_dir(dict.get("win_images"), "/mnt/windows")
        mount_dir(dict.get("linux_images"), "/mnt/images")
